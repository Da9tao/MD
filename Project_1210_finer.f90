    ...
    real(kind=8), parameter :: kb=1.380649d-23, T=298.15d0
    ...
    real(kind=8), parameter :: Q_nh=50d0*4.184d3/N_av*1d-24, Q_nh_null=Q_nh/(E_0*t_0*t_0)
    ...
    ! Second part
    U_first_part = U_first_part - alpha_pi_root * electric_energy_null * DOT_PRODUCT(charge, charge)
    !print *, "U_coumlomb = ", U_first_part
    ...
    integer :: N_molecules=340, N_atoms                                 ! Only water is considered in this case.
    real(8) :: cell_length=6.1d0 , r_cut=3d0, dt=1d-3                   ! r_cut means the coefficient.
    integer :: N_steps
    real(8), allocatable :: position(:,:)
    real(8), allocatable :: velocity(:,:)
    real(8), allocatable :: force(:,:), mass(:), mass_matrix(:,:), k_matrix(:,:), charge(:)
    real(8) :: U_potential, U_kinetic, U_total, pressure, pressure_k, rho_null(2), U_correction(2), r_cut_third, pressure_correction, T_mean, volume, U_accuracy(2), U_error
    real(8) :: R0_temp, D0_temp, mass_total
    real(8) :: alpha, k_radius, r_0_third
    ...
    energy_unit = E_0 * E_unit / real(N_molecules, kind=8)
    ...
    do i = 1, 2
        do j = 1, 2
            if (i == j) then
                ...
                R0_temp = (atom_type(i)%R0 + atom_type(j)%R0) * 0.5d0
            end if
            r_0_third = R0_temp * R0_temp * R0_temp * r_cut_third
            U_correction(1) = U_correction(1) + 4d0/3d0 * pi * D0_temp * (R0_temp*R0_temp*R0_temp) * (rho_null(i)*volume) * rho_null(j) * (1d0/6d0 * (r_0_third*r_0_third*r_0_third) - r_0_third)
            pressure_correction = pressure_correction + 8d0/3d0 * pi * D0_temp * (R0_temp*R0_temp*R0_temp) * rho_null(i) * rho_null(j) * (1d0/3d0 * (r_0_third*r_0_third*r_0_third) - r_0_third)
        end do
    end do
    ...
    k_radius = k_radius * 2d0 * pi / cell_length
    !print "(3(G0,1X))", k_matrix
    !print "(3(G0,1X))", k_matrix==0d0
    
    U_accuracy(1) = electric_energy_null * DOT_PRODUCT(charge, charge) * SQRT(0.5d0 * r_cut / volume) / (alpha * r_cut * r_cut) * EXP(- alpha * r_cut * r_cut)
    U_correction(2) = electric_energy_null * DOT_PRODUCT(charge, charge) * 2d0 * alpha / (k_radius * pi) * EXP(- k_radius * k_radius / (4d0 * alpha))
    U_accuracy(2) = electric_energy_null * DOT_PRODUCT(charge, charge) * SQRT(alpha / pi) * (2d0 / (k_radius * cell_length))**(1.5d0) * EXP(- k_radius * k_radius / (4d0 * alpha))
    U_error = SQRT(U_accuracy * U_accuracy)
    print *, "Accuracy of energy = ", U_error
    ...
    write (*,"(A, I0, A, 7(G0, 1X))") 'Step ', counter, ': ', T_mean*T, U_kinetic*energy_unit, (U_potential)*energy_unit, (U_potential+U_correction(1))*energy_unit, (pressure)*p_0*1d-5, (pressure+pressure_correction)*p_0*1d-5, H_nvt
    write(*,"(3(1X,G0))") center_velocity
    !write(*,*) force

    open(10, file="traj_trial.gro", status="Replace")
    open(20, FILE="recording_trial.txt", status="Replace")
    open(30, FILE="init.dat", status="Replace")
    ...
    ! LAMMPS output
    write(30, "(A)") "LAMMPS Description"
    write(30, "(I0, 1x, A)") N_atoms, "atoms"
    write(30, "(I0, 1x, A)") number_bonds, "bonds"
    write(30, "(I0, 1x, A)") number_angle, "angles"

    write(30, "(/, I0, 1x, A)") 2, "atom types"
    write(30, "(I0, 1x, A)") 1, "bond types"
    write(30, "(I0, 1x, A)") 1, "angle types"

    write(30, "(/, f4.2, 1x, f5.2, 1x, A)") 0.0, cell_length*R_0*1d10, "xlo xhi"
    write(30, "(f4.2, 1x, f5.2, 1x, A)") 0.0, cell_length*R_0*1d10, "ylo yhi"
    write(30, "(f4.2, 1x, f5.2, 1x, A)") 0.0, cell_length*R_0*1d10, "zlo zhi"

    write(30, "(/, A)") "Masses"
    write(30, "(/, I0, 1x, G0)") 1, 16.0
    write(30, "(I0, 1x, G0)") 2, 1.0

    write(30, "(/, A, /)") "Atoms"

    do counter = 1, N_molecules
        do i = 1, 3
            j = molecules_comp(i, counter)
            k = type_matrix(j)
            write(30, "(3(I0, 1X), 3(G0, 1X), G0)") j, counter, k, atom_type(k)%charge, position(:, j)*R_0*1d10
        end do
    end do

    write(30, "(/, A, /)") "Velocities"
    do counter = 1, N_atoms
        write(30, "((I0, 1X), 3(G0, 1X), G0)") counter, velocity(1:3, counter) * R_0 / t_0 * 1d-5
    end do

    write(30, "(/, A, /)") "Bonds"
    do counter = 1, number_bonds
        write(30, "(3(I0, 1X), I0)") counter, 1, pair_bonding(:, counter)
    end do

    write(30, "(/, A, /)") "Angles"
    do counter = 1, number_angle
        write(30, "(4(I0, 1X), I0)") counter, 1, pair_angle((/1, 3, 2/), counter)
    end do
    ...
    do counter = 1, N_steps
        ...
        if (mod(counter, 20)==0) then
            write(20, "(G0,1X,8(G0,1X))") real(counter, kind=8)*dt*t_0, T_mean*T, (pressure)*p_0, (pressure+pressure_correction)*p_0, &
                U_kinetic*energy_unit, U_potential*energy_unit, (U_potential+SUM(U_correction))*energy_unit, (U_total+SUM(U_correction))*energy_unit, &
                H_nvt * energy_unit
